...
- (void)onSuccessWithId:(id<RtdcCoreImplHttpResponse>)resp {
  RtdcCoreModelJsonTransmissionWrapper *wrapper = [[[RtdcCoreModelJsonTransmissionWrapper alloc] initWithNSString:[((id<RtdcCoreImplHttpResponse>) nil_chk(resp)) getContent]] autorelease];
  RtdcCoreServiceService_catchSessionExpiredExceptionWithRtdcCoreModelJsonTransmissionWrapper_(wrapper);
...
- (void)onSuccessWithId:(id<RtdcCoreImplHttpResponse>)resp {
  ...
  RtdcCoreServiceService_catchSessionExpiredExceptionWithRtdcCoreModelJsonTransmissionWrapper_(wrapper);
  if ([@"success" isEqual:[wrapper getStatus]]) {
    RtdcCoreJsonJSONArray *array = [wrapper getDataAsJSONArray];
    ...
    for (jint i = [((RtdcCoreJsonJSONArray *) nil_chk(array)) length] - 1; i >= 0; i--) {
      RtdcCoreModelUnit *unit = [[[RtdcCoreModelUnit alloc] init] autorelease];
      [((id<JavaUtilMap>) nil_chk([unit map])) putAllWithJavaUtilMap:[((RtdcCoreJsonJSONObject *) nil_chk([array getJSONObjectWithInt:i])) map]];
      [units addWithId:unit];
    }
    [((id<RtdcCoreServiceAsyncCallback>) nil_chk(val$callback_)) onSuccessWithId:units];
  }
  else [((id<RtdcCoreServiceAsyncCallback>) nil_chk(val$callback_)) onErrorWithNSString:JreStrcat("$$$", [wrapper getStatus], @" : ", [wrapper getDescription])];
}
